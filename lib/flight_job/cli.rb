...
module FlightJob
  module CLI
    ...
    create_command 'ls-job-dir', 'JOB_ID [--] [LS_OPTIONS...]' do |c|
      ...
      c.description = <<~DESC.chomp
        ...
        #{program(:name)} ls-job-dir JOB_ID -- -laR
      DESC
    end
    ...
    create_command 'view-job-file', signature do |c|
    ...
    unless FlightJob.config.combine_commands
      create_command 'view-job-stdout', 'JOB_ID' do |c|
        ...
        c.action do |args, opts|
          require_relative '../flight_job'
          opts.stdout = true
          FlightJob::Commands::ViewJobFile.new(args, opts).run!
        end
      end
